...
commander_1.default
    ...
    try {
        const allBrowsers = new Set(registry_1.allBrowserNames);
        for (const browserType of browserTypes) {
            if (!allBrowsers.has(browserType)) {
                console.log(`Invalid browser name: '${browserType}'. Expecting one of: ${registry_1.allBrowserNames.map(name => `'${name}'`).join(', ')}`);
                process.exit(1);
            }
        }
        if (browserTypes.length && browserTypes.includes('chromium'))
            browserTypes = browserTypes.concat('ffmpeg');
        await driver_1.installBrowsers(browserTypes.length ? browserTypes : undefined);
    }
    catch (e) {
